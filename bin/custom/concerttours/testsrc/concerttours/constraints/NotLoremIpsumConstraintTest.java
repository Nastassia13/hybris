...
public class NotLoremIpsumConstraintTest extends ServicelayerTransactionalTest {
    ...
    public void testLoremIpsumConstraint() {
        ...
        Set<HybrisConstraintViolation> violations = validationService.validate(band);
        Assert.assertTrue("The violation set should not be null or empty",
        ...
        for (HybrisConstraintViolation violation : violations) {
            Assert.assertEquals(NotLoremIpsumConstraintModel.class, violation.getConstraintModel().getClass());
        }
    }
}